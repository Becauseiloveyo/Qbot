...
class Database:
    ...
    def _backup_before_migration(self) -> Path:
        ...
        return self.create_backup(backup)

    def create_backup(self, destination: Path | None = None) -> Path:
        """Create a consistent SQLite snapshot without mutating Qbot state."""

        if not self.db_path.exists():
            raise RuntimeError("cannot back up missing database")

        if destination is None:
            stamp = datetime.now(UTC).strftime("%Y%m%dT%H%M%S%fZ")
            destination = self.db_path.with_name(
                f"{self.db_path.name}.snapshot-{stamp}"
            )
    ...
    def diagnose(self) -> DatabaseDiagnostic:
        """Read-only database health report; never migrates or writes metadata."""

        if not self.db_path.exists():
            return DatabaseDiagnostic(
                exists=False,
                schema_version=None,
                target_schema_version=self.config.db_schema_version,
                integrity_ok=False,
        ...
        try:
            with self.engine.connect() as conn:
                ...
                if "qbot_meta" not in tables:
                    issues.append("qbot_meta is missing")
                else:
                    ...
                    if schema_version is None:
                        issues.append("db_schema_version is missing")
                    else:
                        current = int(schema_version)
                        target = int(self.config.db_schema_version)
                        if current < target:
                            issues.append(
                                f"migration required: {current} -> {target}"
                            )
                        elif current > target:
                            issues.append(
                                f"database schema {current} is newer than supported {target}"
                            )
                        else:
                            missing = sorted(set(metadata.tables) - tables)
                            if missing:
                                issues.append(
                ...
                if not integrity_ok:
                    issues.append(
                        "integrity_check: " + "; ".join(str(x) for x in integrity)
                    )

                foreign_errors = conn.execute(
                    text("PRAGMA foreign_key_check")
                ).all()
                foreign_keys_ok = not foreign_errors
                if foreign_errors:
                    issues.append(
                        f"foreign_key_check: {foreign_errors!r}"
                    )
        except Exception as exc:
